...
import { EventMarketplace } from '../target/types/event_marketplace'
import { LAMPORTS_PER_SOL, Connection, PublicKey } from '@solana/web3.js'
import assert from 'assert'
import BN from 'bn.js'


describe('event_marketplace', () => {
  anchor.setProvider(anchor.AnchorProvider.env())

  const program = anchor.workspace.event_marketplace as Program<EventMarketplace>
...
async function createEvent(
  program: Program<EventMarketplace>,
...
async function buyTicket(program: Program<EventMarketplace>, buyer: anchor.web3.Keypair, ticket: Ticket) {
...
async function verifyTicket(program: Program<EventMarketplace>, ticket: Ticket) {
  const [ticketPda] = getTicketPda(program, ticket.event, ticket.buyer)
...
function getTicketPda(program: Program<EventMarketplace>, eventPda: PublicKey, buyer: PublicKey): [PublicKey, number] {
  return anchor.web3.PublicKey.findProgramAddressSync(
