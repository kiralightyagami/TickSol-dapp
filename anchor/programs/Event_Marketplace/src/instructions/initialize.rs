...
pub fn _initialize(ctx: Context<InitializeContext>, name: String, description: String, ticket_price: u64, available_tickets: u64, start_date: i64) -> Result<()> {
    ...
    require!(
        ...
        EventMarketplaceError::NameTooLong
    );
    event.name = name;

    require!(
        description.len() <= MAX_DESCRIPTION_LEN,
        EventMarketplaceError::DescriptionTooLong
    );
    event.description = description;

    require!(
        available_tickets > 0,
        EventMarketplaceError::AvailableTicketsTooLow
    );
    event.available_tickets = available_tickets;

    require!(
        start_date > Clock::get()?.unix_timestamp,
        EventMarketplaceError::StartDateInThePast
    );
